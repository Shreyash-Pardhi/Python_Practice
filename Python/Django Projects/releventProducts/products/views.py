...
def Input_IMG_features(img_path):
    ...
    return check_relevant_products(txt)

def check_relevant_products(inp_features):
    global error
    try: 
        name = []
        link = []
...
def relevantProd(req):
    if req.method == 'POST':
        uri = req.POST.get("url_Link")
        if(uri=='' or is_url_image(uri)==False):
            return render(req,'Error.html', {"erro": error})
        li,nam = Input_IMG_features(uri)
        items = [{"link":l, "name":n} for l,n in zip(li,nam)]
        if len(li)==0:
            return render(req,'Error.html', {"erro": "No product found in dataBase, Please try again later"})
