...
def Feature_Extraction(img_path):
    ...
    try:
        ...
        objects = client.object_localization(image=img).localized_object_annotations
        object = [o.name for o in objects]
        ob = f"Object not detected!!" if len(object)==0 else f"{object}"
...
with gr.Blocks(title="Image-Feature Extraction",
        ...
        gr.Interface(fn= Feature_Extraction,
                        ...
                        outputs= [brand, labels, text, obj],
                        title="<p style='color:orange; font-size:35px'>Image Feature Extraction</p>",
                        description="<p style='font-size:15px'>Upload an Image, and see extracted features from image using Google Vision API.<br>Features Like; Logo Detection, Labels Detection, Text Detection and Objects Detection.</p>",
                        allow_flagging='never',
                    )

features.launch(debug=True)